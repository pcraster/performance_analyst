...
class TimerResult(dict):
    ...
    The first item is the time stamp of the moment the timer case
    finished. The second item is the description or the :meth:`timer
    case's id <performance_analyst.TimerCase.TimerCase.id>` if no
    description was found. The third item is a list of timings, one
    tuple of (real_time, cpu_time), for each time the timer case was
    run. Timings are in seconds.
    """
    ...
    def __str__(self):
        ...
        for key in self:
            ...
            for timings in self[key][2]:
                ...
                result.append("{} {} {}".format(str(self[key][0]),
                    self[key][1], " ".join(str(timing) for timing in timings)))
